...
App.controller.TestvectorController = function () {
    ...
    this.testcontentModel = new App.models.TestcontentModel();
...
App.controller.TestvectorController.prototype.actionCreate = function () {
    ...
    promises.push(self.testcontentModel.getElements());

    Q.all(promises)
      .then(function (result) {
          return self.view.renderCreate({attributes: result[0], testcontents: result[1]});
      })
...
App.controller.TestvectorController.prototype.actionEdit = function (id) {
    ...
    if (!id) {
        ...
        Q.all(promises)
          .then(function (result) {
              self.view.renderEdit({element: result[0], testcontents: result[1],});
          })
          .catch(function (error) {
              App.handler.Errorhandler.handleError(error);
          });
